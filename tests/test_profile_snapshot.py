...
def _write_profile(profile_dir, *, metric_text: str, evidence: int, extra_project=False):
    ...
    matrix = {
        "rows": [
            {"key": "python", "display": "Python", "evidence_fact_ids": [f"f{i}" for i in range(evidence)]}
        ]
    }
...
def test_diff_is_deterministic_and_reports_gains(tmp_path):
    ...
    _write_profile(after_dir, metric_text="Cut deploy time 40%.", evidence=3, extra_project=True)
    diff = snapshot_diff(profile_snapshot(before_dir), profile_snapshot(after_dir))
    assert diff["newFactIds"] == ["p1"]
    assert diff["bulletsGainedMetrics"] == [{"experienceId": "e1", "before": 0, "after": 1}]
    assert diff["skillsGainedEvidence"] == [{"skill": "python", "before": 1, "after": 3}]

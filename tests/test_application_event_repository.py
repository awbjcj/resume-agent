...
def test_events_are_ordered_by_date_ascending():
    ...
    for day, kind in ((9, "online_assessment"), (3, "recruiter_screen")):
        save_application_event(
            session,
            ApplicationEvent(application_id=app.id, kind=kind, occurred_at=_at(day)),
        )
...
def test_undated_events_sort_last():
    ...
    save_application_event(
        session,
        ApplicationEvent(application_id=app.id, kind="custom", custom_label="note"),
    )
    save_application_event(
        session,
        ApplicationEvent(
            application_id=app.id, kind="recruiter_screen", occurred_at=_at(3)
        ),
    )
    kinds = [e.kind for e in events_for_application(session, app.id)]
...
def test_next_sequence_counts_only_the_same_kind():
    ...
    save_application_event(
        session,
        ApplicationEvent(
            application_id=app.id, kind="technical_round", occurred_at=_at(3)
        ),
    )
    save_application_event(
        session,
        ApplicationEvent(application_id=app.id, kind="behavioral", occurred_at=_at(4)),
    )
...
def test_delete_removes_the_row():
    ...
    event = save_application_event(
        session,
        ApplicationEvent(application_id=app.id, kind="behavioral", occurred_at=_at(3)),
    )
...
def test_events_are_scoped_to_one_application():
    ...
    save_application_event(
        session,
        ApplicationEvent(
            application_id=other.id, kind="behavioral", occurred_at=_at(3)
        ),
    )
    assert events_for_application(session, app.id) == []

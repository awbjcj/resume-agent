...
def test_builders_keep_router_and_formatter_tool_free(monkeypatch):
    ...
    monkeypatch.setattr(agents, "get_settings", lambda: SimpleNamespace(
        cheap_model="cheap", mid_model="mid"
    ))
...
def test_persona_builder_attaches_one_verified_local_skill(monkeypatch):
    ...
    monkeypatch.setattr(agents, "get_settings", lambda: SimpleNamespace(mid_model="mid"))

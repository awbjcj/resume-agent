...
def test_analytics_table_rows_formats_counts_and_rates():
    with _session() as session:
        job = save_job(
            session, Job(source="greenhouse", company="C", title="T", fit_score=85, status="rendered")
        )
        assert job.id is not None
        save_application(session, Application(job_id=job.id, status=ApplicationStatus.interview.value))

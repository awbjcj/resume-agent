...
def test_fresh_boot_seeds_and_links_mutable_paths(tmp_path):
    ...
    assert (data_root / "config" / "review.yaml").read_text(encoding="utf-8") == "max_rounds: 2\n"
...
def test_redeploy_preserves_data_and_is_idempotent(tmp_path):
    ...
    (data_root / "config" / "review.yaml").write_text("max_rounds: 9\n", encoding="utf-8")
    (data_root / ".env").write_text("KEY=value\n", encoding="utf-8")

    prepare_data_root(app_root, data_root, defaults)

    assert "edited" in (data_root / "config" / "search.yaml").read_text(encoding="utf-8")
    assert (data_root / "config" / "review.yaml").read_text(encoding="utf-8") == "max_rounds: 9\n"

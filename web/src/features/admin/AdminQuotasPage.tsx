  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <section aria-labelledby="platform-runway-title" className="overflow-hidden rounded-xl border bg-card shadow-sm">
        ...
        <div className="p-5">
          <dl className="grid gap-5 sm:grid-cols-2 xl:grid-cols-5"><Metric label="Month spend" value={usd(summary.data.monthlySpendMicros)} detail="Shared platform keys" /><Metric label="Platform cap" value={usd(summary.data.monthlyCapMicros)} detail="UTC calendar month" /><Metric label="Runway" value={usd(summary.data.remainingMicros)} detail={`${Math.max(0, 100 - runway).toFixed(1)}% remains`} warning={runway >= 80} /><Metric label="Unpriced calls" value={String(summary.data.unpricedCallCount)} detail="Requires rate coverage" warning={summary.data.unpricedCallCount > 0} /><Metric label="Next reset" value={new Date(summary.data.nextResetAt).toLocaleDateString(undefined, { month: "short", day: "numeric" })} detail="00:00 UTC" /></dl>
          <Progress className="mt-6" value={Math.min(100, runway)}><ProgressLabel>Shared-key monthly cap</ProgressLabel><ProgressValue>{() => `${runway.toFixed(1)}%`}</ProgressValue></Progress>
        </div>
      </section>
      <BulkCommandBar accounts={accounts.data.data} />
      <Tabs defaultValue="members">
        <div className="overflow-x-auto border-b">
          <TabsList className="h-11 gap-5" variant="line" aria-label="Quota console sections"><TabsTrigger className="px-1" value="members"><Users /> Members</TabsTrigger><TabsTrigger className="px-1" value="tiers"><Layers3 /> Tiers</TabsTrigger><TabsTrigger className="px-1" value="rates"><Gauge /> Rate cards</TabsTrigger><TabsTrigger className="px-1" value="audit"><History /> Audit</TabsTrigger></TabsList>
        </div>
        <TabsContent value="members" className="space-y-4 pt-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div className="flex w-full max-w-xl flex-col gap-2 sm:flex-row sm:items-center"><div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><Input className="pl-9" aria-label="Search members" placeholder="Search member…" value={search} onChange={(event) => setSearch(event.target.value)} /></div><select aria-label="Balance filter" className="h-9 rounded-lg border bg-background px-3 text-sm" value={balanceFilter} onChange={(event) => setBalanceFilter(event.target.value)}><option value="">All balances</option><option value="POSITIVE">Positive</option><option value="ZERO">Depleted</option><option value="OVERAGE">Overage</option><option value="UNLIMITED">Unlimited</option></select></div><p className="text-xs text-muted-foreground">Member accounts only · administrators are quota-exempt</p></div><Card className="overflow-hidden"><CardContent className="overflow-x-auto p-0"><Table><TableHeader><TableRow><TableHead>Member</TableHead><TableHead>Tier</TableHead><TableHead>Period spend</TableHead><TableHead>Remaining</TableHead><TableHead>Credits</TableHead><TableHead>Shared / BYOK</TableHead><TableHead>Tokens</TableHead><TableHead>Reset</TableHead></TableRow></TableHeader><TableBody>{filtered.map((account) => <TableRow key={account.userId} className="cursor-pointer" tabIndex={0} onClick={() => setSelected(account)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelected(account); }}><TableCell><div className="font-medium">{account.username}</div>{account.disabled ? <span className="text-xs text-muted-foreground">Disabled</span> : null}</TableCell><TableCell><Badge variant="outline">{account.tierId}</Badge></TableCell><TableCell className="font-mono">{usd(account.spentMicros)}</TableCell><TableCell className="font-mono">{usd(account.remainingMicros)}</TableCell><TableCell className="font-mono">{usd(account.creditBalanceMicros)}</TableCell><TableCell className="font-mono">{usd(account.sharedCostMicros)} / {usd(account.byokCostMicros)}</TableCell><TableCell className="font-mono">{tokens(account.totalTokens)}</TableCell><TableCell className="font-mono">{new Date(account.periodEnd).toLocaleDateString()}</TableCell></TableRow>)}</TableBody></Table>{filtered.length === 0 ? <div className="px-6 py-12 text-center"><p className="font-medium">No member accounts match</p><p className="mt-1 text-sm text-muted-foreground">Adjust the search or balance filter to see more accounts.</p></div> : null}</CardContent></Card></TabsContent>
        <TabsContent value="tiers"><TierPanel tiers={tiers.data.data} /></TabsContent>
        <TabsContent value="rates" className="pt-4"><RateCreator /><Card><CardHeader><CardTitle><h3>Effective rate versions</h3></CardTitle><CardDescription>Historical rows are immutable. Coverage includes exact provider, model, context band, and effective window.</CardDescription></CardHeader><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Provider / model</TableHead><TableHead>Context band</TableHead><TableHead>Input</TableHead><TableHead>Cache read / write</TableHead><TableHead>Output</TableHead><TableHead>Tool fee</TableHead><TableHead>Effective</TableHead></TableRow></TableHeader><TableBody>{rates.data?.data.map((rate) => <TableRow key={rate.id}><TableCell><div className="font-medium">{rate.provider}</div><div className="font-mono text-xs text-muted-foreground">{rate.model}</div></TableCell><TableCell className="font-mono">{tokens(rate.contextMinTokens)}–{rate.contextMaxTokens == null ? "∞" : tokens(rate.contextMaxTokens)}</TableCell><TableCell className="font-mono">{usd(rate.inputMicrosPerMillion)}</TableCell><TableCell className="font-mono">{usd(rate.cacheReadMicrosPerMillion)} / {usd(rate.cacheWriteMicrosPerMillion)}</TableCell><TableCell className="font-mono">{usd(rate.outputMicrosPerMillion)}</TableCell><TableCell className="font-mono">{usd(rate.toolMicrosPerUnit)}</TableCell><TableCell className="font-mono"><div>{new Date(rate.effectiveFrom).toLocaleDateString()}</div><div className="text-xs text-muted-foreground">{rate.effectiveTo ? `to ${new Date(rate.effectiveTo).toLocaleDateString()}` : "active"}</div></TableCell></TableRow>)}</TableBody></Table></CardContent></Card></TabsContent>

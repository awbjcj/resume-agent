# Résumé Tailor Harness threat model

## Executive summary

The source audit originally found that Résumé Tailor Harness was not ready for unrestricted public registration. The P0 code blockers are now remediated on `codex/security-blockers`; deployment still requires the canonical Railway origin, working mail delivery, explicit budgets, and a live smoke test before opening the domain.
...
- Create and control many email accounts and, after open registration, many Résumé Tailor Harness accounts.

...
Deploy resume-tailor-harness to Railway as a personal (single-tenant) instance reachable
...
`resume-tailor-harness serve --host 0.0.0.0 --port $PORT`. Railway healthcheck:
...
- `auth_password_hash: str = ""` — PBKDF2-HMAC-SHA256, format
  ...
  (`resume-tailor-harness hash-password`). Stdlib `hashlib.pbkdf2_hmac`, no new
  dependency.
...
| Path      | Handling                                                                                                                                                                                                                                                          |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `data/`   | The volume mount point itself. Holds the SQLite DB (`db_url` default already `sqlite:///data/resume_tailor_harness.db`), profile corpus/facts, runs, workday facets, scraper recipes.                                                                                      |
...
`src/resume_tailor_harness/deploy.py` — pure function, unit-testable against tmp dirs —
...
- `GET /api/admin/export` — streams `resume-tailor-harness-data-<date>.tar.gz` of the

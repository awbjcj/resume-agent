...
- **Success:** for N independent units, wall-clock approaches
  ...
  bounded by a global concurrency cap. Results and final DB state are identical to
  the serial path. All existing tests pass; new tests prove the cap, error
  isolation, ordering, and a measurable speedup with delayed fakes.
...
- Wraps each `fn(item)` in a shared `asyncio.Semaphore(limit)`.
- Drives completion with `asyncio.as_completed` so a progress counter can step as
  each task finishes (completion order is nondeterministic).
...
### Async containment at the service boundary

The async stays behind `services/`. `services/discovery.py` and
`services/tailoring.py` keep their **sync** signatures; internally they call
`asyncio.run(_discover_async(...))` / `asyncio.run(_tailor_async(...))`. `cli.py`
...
`await map_concurrent(...)` the pure LLM fn → **(c)** apply results + commit
...
- `tailor_jobs` gathers `run_tailor_review` (already pure) across jobs via
  `map_concurrent`.
- `run_panel` gathers its reviewers via `map_concurrent`.
- Both share the **same** global semaphore, so nested fan-out (jobs × reviewers)
  cannot exceed `llm_concurrency`.
...
| --- | --- | --- |
| `llm_concurrency` | `8` | Max in-flight LLM calls across all phases (semaphore size). |
| `llm_retries` | `2` | agno `retries` per agent. |
| `llm_retry_delay` | `1` | agno `delay_between_retries` (seconds), with exponential backoff. |

One semaphore is created per `asyncio.run` invocation, sized from
`llm_concurrency`, and threaded into `map_concurrent`.

## Progress reporting under concurrency

`ProgressReporter.step(current)` is driven from the `as_completed` loop: step a
running completed-count, not the input index. ETA stays honest (rate =
...
- Per-item failures isolated by `map_concurrent` (matches current behaviour).
...
- Results applied in deterministic input order → DB state identical to serial.

## Testing

- `concurrency.map_concurrent`:
  - cap enforced (max simultaneously-running observed ≤ `limit`);
...
- Per phase (discovery + tailor): a fake agent with an artificial `await` delay
  ...
  `limit=N` is meaningfully below the serial sum; a throwing fake proves

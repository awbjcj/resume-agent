...
**Goal:** Make a Coach / Interviewer / Scout turn _feel_ as fast as the provider actually is. Cut ~200–330 ms of self-inflicted transport latency per visible chunk, remove ~34 characters of permanent lag behind the model's cursor, stop the caret blinking through a 1–4 s formatter call the user cannot see, and make the browser's cost-per-delta independent of durable transcript length.
...
## Measured baseline and deterministic after measurements

The offline harness uses a controlled 10 ms delta cadence. Provider TTFT and formatter
duration remain intentionally live-only and are reported by `scripts/stream_latency.py`.

| Stage | Baseline p50 | Baseline p95 | After observed |
| --- | --- | --- | --- |
| Prose holdback behind model cursor | 34 chars | 34 chars | 0 chars on boundary-free prose |
| Sink batch window | 40 ms | 70 ms | 20 ms p50 / 30 ms p95; 30 rows vs 15 |
| Log handle opens | 1 per row | 1 per row | 1 per run; every row still flushed |
| **SSE poll wait** | 125 ms | 237.5 ms | 0 sleeps on notified happy path |
| Terminal grace | 1,000 ms | 1,000 ms | 200 ms maximum fallback grace |
| Browser text renders/event at M=20 | 21 | 21 | 1 changing message |
| **Post-prose dead tail** | live probe required | live probe required | caret clears at `settled`; duration still reported live |
| Per-turn link-token mint | 1 | 1 | 0 while cached token is warm |
...
- [x] Add `tests/perf/test_stream_latency.py` (offline, deterministic) around the real `RunStreamSink` and `stream_events` generator. `httpx`'s in-process ASGI transport buffers the response body and cannot expose per-yield timing, so route integration remains covered by `test_run_stream_route.py` while the perf test injects the wait seam directly. It proves the notified path takes no poll sleep and the dropped-notification path still drains the log.
- [x] Add `scripts/stream_latency.py`: an opt-in live probe (requires a real key, never runs in CI) that drives one Coach turn and reports **TTFT**, inter-chunk gap p50/p95, total events, total log bytes, and the wall-clock split between _prose end_ and _run completed_. This last number is the Task 4 justification.
  **Implementation correction (2026-08-02):** report received SSE event bytes rather
  than server log-file bytes. The probe supports a remote `--base-url`, where the
  server's filesystem is intentionally unavailable; wire bytes are the portable
  transport-cost metric.
- [x] Web: add a render-count harness to `web/src/lib/chat/useChatStream.test.ts` (or a new `ChatThread.perf.test.tsx`) that feeds K events into a thread of M durable messages and counts `TextPart` renders. Today this is O(K·M); Task 6 makes it O(K).
- [x] Fill the **Measured baseline** table above. This is the row every later task is compared against.
...
- [x] Add a `StreamNotifier` to `api/runs/manager.py` (or a new `api/runs/notify.py`): a per-run object holding a set of `asyncio.Event`s plus the loop each was created on. `notify()` is thread-safe and fans out via `call_soon_threadsafe`; `subscribe()` returns an event a generator can await; unsubscribe on generator close.
- [x] Give `RunStreamSink` an optional `on_append: Callable[[], None]` invoked after each row is durably written (after `flush()`, never before — a reader woken early would find nothing and busy-spin). `with_conversation_stream` wires it to `manager.notifier(run_id).notify`.
- [x] Rewrite the `stream_events` loop: drain `tail.read(cursor)`; if it produced nothing, `await asyncio.wait_for(event.wait(), timeout=poll_interval)` instead of `await asyncio.sleep(poll_interval)`. **Keep the timeout.** The poll remains the correctness path — the notifier is a latency optimisation, and a dropped notification must cost 250 ms, not a hang.
- [x] Drop `poll_interval` for the _terminal grace_ path to 0.05 s. It only runs after the manager already reports a terminal state, so the four polls exist to catch a straggling row, not to wait on work.
...
- [x] Replace the fixed-window flush in `ProseEmitter.feed` with a `_safe_prefix_len(pending)` helper: return the index of the earliest position from which a boundary match could still complete, or `len(pending)` when none exists. Flush everything before it.
- [x] Keep `MARKER_MAX_LEN` as the hard ceiling on how far back a candidate may start — the helper must never scan unboundedly into the buffer.
- [x] Extend `tests/test_turn_prose.py`: prose ending mid-word flushes fully; prose ending in `\n\n` withholds from the blank line; prose ending in `\n\n--` withholds from the blank line; a boundary split across three deltas is still caught and still truncates at the right place; **every existing leak test still passes** (this is the gate — re-run the `PROPOSE |`, bare-`---`-rule, and `**Action:**`-mid-sentence cases explicitly).
...
- [x] Add a `Settled` event to `sessions/stream.py` (`tag = "settled"`, empty payload). **Do not** add it to `TERMINAL_TAGS`.
- [x] Emit it from `persona_output` immediately after `emitter.finish()`, guarded on `prose` being non-empty — when nothing streamed (`stream_enabled` off, or a non-streaming agent) the reply genuinely does arrive from the formatter and must keep the caret.
- [x] Mirror the tag in `web/src/lib/chat/events.ts` (`STREAM_EVENT_TAGS`, `StreamEvent`, `parseStreamEvent`) and add a `"settled"` status to `useChatStream`: the EventSource **stays open** (a `notice` and the terminal still follow), but `status` leaves `"streaming"`.
- [x] Update `CoachPage`, `InterviewPage`, and `ScoutPage`: `busy` stops including `stream.status === "settled"`, and `ChatMessage`'s `streaming` prop (which drives the caret) goes false. The run may still be `"running"` — show a quiet secondary indicator ("saving…") rather than a blocked composer, so a user who wants to type their next message can.
...
- [x] Update the Python↔TS tag parity test so a missing mirror fails CI.
...
- [x] `RunStreamSink` opens its handle once (lazily, on first append) and holds it until `close()`, writing + `flush()` per row. `flush()` stays — `StreamTail` relies on complete newlines being visible to a separate reader.
- [x] `close()` must close the handle on every path, including the exception paths in `with_conversation_stream`.
- [x] Confirm `StreamTail` still reads correctly from a file with a live open append handle (it opens `"rb"` independently — verify on Windows specifically, where sharing semantics differ from POSIX).
...
## Task 6 — Make browser cost per delta independent of transcript length
...
- [x] Wrap `ChatMessage` in `React.memo`. Durable messages come from query data with
      stable identity, so only the streaming message re-renders.
- [x] Validate and reject the proposed blank-line `TextPart` split because it changes
      CommonMark semantics; retain one whole-message parse for the changing message and
      memoise durable messages instead.
- [x] Move the auto-scroll effect's `scrollHeight` read into `requestAnimationFrame` so
      it stops forcing synchronous layout inside React's commit phase.
- [x] `ReasoningPart` renders plain text inside a **collapsed** `Collapsible`. Confirm
...
- [x] Cache the SSE link token in a module-scoped ref with its expiry; reuse it until it
      is near expiry or the connection is rejected, then re-mint once.
- [x] A native browser `EventSource` error does not expose the HTTP status. When the
...
- [x] Keep the token purpose-bound to SSE (ADR-0003: query tokens are never general API
...
- [x] Pass `cache_system_prompt=capabilities.supports_prompt_cache` to the coach,
      interviewer, and formatter builders, matching the existing call sites exactly.
- [x] Audit prompt _ordering_ for cache-prefix stability. `run_message_turn` composes
    ...
    agenda → user message`.
- [x] Consider moving `_overview(root, engine)` — stable for a whole session — into the
...
- [x] Re-run the Task 1 harness at (0.08, 240) — the current values — and at (0.04, 120)
      and (0.02, 60). Record events/turn, log bytes/turn, and render count/turn for each.
- [x] Pick the smallest interval whose event count stays within ~2× today's and whose
...
- [x] Keep reasoning and text on separate budgets — that rule is load-bearing (a kind
...
- [x] Update `CLAUDE.md`'s streaming section: the transport is now notify-with-poll-fallback;
...
- [x] Evaluate whether Task 2 or 4 needs an ADR. None was added because the NDJSON
      durability boundary remains unchanged and the notifier is only an optimisation.
- [x] Keep the perf harness under `tests/perf`; the existing full `pytest tests` gate
      discovers it, and this repository has no `dev` branch/job to modify.
- [x] Fill the **Measured baseline** table's after-column. Paste the before/after into
      the PR description when a PR is opened.
...
| Browser cost per delta            | O(whole transcript)                   | O(one changing message)   |
...
Tasks 1–7, Task 8's offline cache wiring and prompt-order audit, Task 9, and the
Task 10 documentation/regression guards are implemented on
`perf/streaming-fluency`. The deterministic harness supplies the after measurements
above, and the perf tests are discovered by the existing full `pytest tests` CI gate,
so no separate CI command was needed. No ADR was added because the implementation
preserves the existing NDJSON durability boundary and introduces no competing source
of truth.

Two items remain deliberately live-only: running `scripts/stream_latency.py` against
...
into the chat window; too cautious just keeps some lag. The implementation therefore
uses the **conservative** policy: boundary-free prose flushes fully, while partial
marker and blank-line candidates retain a bounded safety margin. Existing and new leak
regressions cover bare rules, action labels, and Scout proposal rows.

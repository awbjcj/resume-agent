...
## Skill taxonomy and Career Capability Matrix
...
| `CAREER_CAPABILITY_MODE` | `legacy` | `legacy`, `shadow`, or `uccm`. `shadow` computes UCCM beside primary legacy results. |
| `UCCM_EVALUATION_REPORT_PATH` | `data/evals/uccm_activation_report.json` | Reviewed activation report used when `CAREER_CAPABILITY_MODE=uccm`. |

`uccm` is fail-closed: it becomes the effective primary mode only when the
activation report is complete, reviewed, current, sealed, threshold-passing,
and pinned to the active taxonomy/assertion/extraction/matching revisions.
Otherwise the effective mode is `shadow`. See the
[controlled-switch note](notes/2026-08-19-uccm-controlled-switch.md).

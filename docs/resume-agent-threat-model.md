...
6. OAuth state is signed but not stored, atomically consumed, or bound to the initiating browser, while callback construction accepts forwarded host/proto values. This weakens login-flow integrity.
...
| TM-005 | OAuth login/session swapping or redirect confusion                          | Account identities and sessions                                  | HMAC-signed, expiring state                                                            | State is stateless/replayable and not browser-bound; forwarded host/proto build callback                                             | Store a hashed one-time state nonce tied to an initiating browser cookie/session; consume atomically; use configured callback URIs only                                                                                                               | P1                    |
